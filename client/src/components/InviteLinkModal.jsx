...
export default function InviteLinkModal({ roomId, roomCode, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-surface-container-high border border-outline-variant/20 rounded-2xl w-full max-w-md p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mb-4">
          ...
          <div className="bg-surface border border-outline-variant/30 rounded-lg px-4 py-2 text-on-surface font-mono text-lg tracking-wider">
            {roomCode}
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-2">
            <input
              readOnly
              value={inviteLink}
              className="flex-1 bg-surface border border-outline-variant/30 rounded-lg px-4 py-2 text-on-surface text-sm font-mono focus:outline-none focus:border-primary"
            />
            <button
              onClick={copyLink}
