...
const QUALITY_LABELS = [
  { value: 0, label: 'Again', color: 'bg-red-600 hover:bg-red-500' },
  { value: 1, label: 'Hard', color: 'bg-orange-600 hover:bg-orange-500' },
  { value: 2, label: 'Good', color: 'bg-yellow-600 hover:bg-yellow-500' },
  { value: 3, label: 'Easy', color: 'bg-green-600 hover:bg-green-500' },
  { value: 4, label: 'Great', color: 'bg-blue-600 hover:bg-blue-500' },
  { value: 5, label: 'Perfect', color: 'bg-purple-600 hover:bg-purple-500' },
]
  ...
  return (
    <div className="min-h-screen bg-surface text-on-surface p-8">
      <div className="max-w-3xl mx-auto">
          ...
          {['list', 'review', 'create'].map((m) => (
            <button key={m} onClick={() => { setMode(m); setEditId(null); setFront(''); setBack('') }}
              className={`px-4 py-2 rounded-xl text-sm font-medium capitalize transition ${
                mode === m ? 'bg-primary text-on-primary' : 'bg-white text-on-surface/60 hover:bg-white/80'
              }`}>
        ...
        <AnimatePresence mode="wait">
          ...
          {mode === 'review' && currentCard && (
            <motion.div key="review" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div
                className="bg-white rounded-2xl shadow-lg p-12 min-h-[300px] flex items-center justify-center cursor-pointer select-none mb-6"
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-6 gap-2">
                  {QUALITY_LABELS.map((q) => (
                    <button key={q.value} onClick={() => reviewCard(q.value)}
                      className={`py-3 rounded-xl text-white font-medium text-sm transition ${q.color}`}>
                      {q.label}
                    </button>
                  ))}
                </motion.div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <textarea value={front} onChange={(e) => setFront(e.target.value)}
                      className="w-full bg-surface border border-outline-variant rounded-xl px-4 py-3 text-on-surface focus:outline-none focus:border-primary h-24 resize-none"
                  ...
                  <div className="flex gap-3">
                    ...
                    {editId && (
                      <button onClick={() => { setEditId(null); setFront(''); setBack(''); setMode('list') }}
                        className="px-6 py-2 bg-surface text-on-surface/60 rounded-xl font-medium hover:bg-surface-container transition">
                        Cancel
                      </button>
                    )}
                  </div>
            ...
            <motion.div key="list" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              {dueCards.length > 0 && (
                <button onClick={startReview}
                  className="w-full mb-6 py-4 bg-primary text-on-primary rounded-2xl font-bold text-lg hover:bg-primary/90 transition shadow-lg">
              ...
              {flashcards.length === 0 ? (
                ...
                <div className="space-y-2">
                  {flashcards.map((card) => (
                    <div key={card._id} className="bg-white rounded-xl p-4 flex items-start justify-between shadow-sm">
                      <div className="flex-1 min-w-0 mr-4">
                        ...
                        <div className="flex gap-3 mt-1 text-xs text-on-surface/40">
                          {card.isMastered && <span className="text-green-500">Mastered</span>}
                      ...
                      <div className="flex gap-1">
                        <button onClick={() => editCard(card)}
                          className="p-2 hover:bg-surface rounded-lg text-on-surface/40 hover:text-on-surface transition text-sm">
                          ✏️
                        </button>
                        <button onClick={() => deleteCard(card._id)}
                          className="p-2 hover:bg-surface rounded-lg text-on-surface/40 hover:text-red-500 transition text-sm">
                          🗑️
                        </button>
                      </div>
                    </div>
        ...
        <Link to="/dashboard" className="inline-block mt-8 text-primary hover:text-primary/80 transition">
          ← Back to Dashboard
        </Link>
      </div>
    </div>
